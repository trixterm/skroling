...
window.FluidSim = function(canvasId, options) {
  ...
  options.initVFn = options.initVFn || [ '0', '0'];
  ...
  var WIDTH = options.size.width || 400;
  var HEIGHT = options.size.height || 400;
  var EPSILON = 1/WIDTH;
  ...
  var gl = GL.create(canvas, {alpha:true});
  ...
  var drawBlack = makeFunctionPainter('1.0', '1.0', '1.0', '1.0');
  ...
  var drawTexture = (function() {
    var shader = new gl.Shader(standardVertexShaderSrc, `
      ...
      void main() { 
        ...
        gl_FragColor = (texture2D(inputTexture, textureCoord) + vec4(1.0)) / 2.0;
        gl_FragColor = mix(gl_FragColor, vec4(vec3(length(gl_FragColor)/2.0), 1.0), 0.5);
      } 
    `);
  ...
  gl.ondraw = function() {
    ...
    if (options.threshold) {
      ...
      drawTexture(textures.velocity0);
    }
...
window.onload = runFluidSim(windowSize);
window.onresize = runFluidSim(windowSize);

function runFluidSim (windowSize) {
  new FluidSim("fluid-canvas", {
      threshold: false,
      advectV: true,
      applyPressure: true,
      showArrows: false,
